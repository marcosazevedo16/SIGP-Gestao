...
let currentReportPDF = null; // Guarda o PDF gerado

function generateCustomReportPreview() {
    const filters = {
        startDate: document.getElementById('report-start-date').value,
        endDate: document.getElementById('report-end-date').value,
        module: document.getElementById('report-module').value,
        municipios: Array.from(document.getElementById('report-municipios').selectedOptions).map(o => o.value),
        status: document.getElementById('report-status').value,
        collaborator: document.getElementById('report-collaborator').value.trim()
    };

    // Validação básica
    if (!filters.startDate || !filters.endDate) {
        showToast('Selecione o período inicial e final', 'error');
        return;
    }

    // Aqui você filtra os dados reais (vou simular com exemplo)
    const data = filterDataByCriteria(filters); // ← você já tem os arrays globais
...
function generatePDFReport(data, filters) {
    ...
    // Cabeçalho bonito
    doc.setFontSize(18);
    doc.text('SIGP Saúde - Relatório Gerencial', pageWidth / 2, y, { align: 'center' });
    y += 10;
    doc.setFontSize(12);
    doc.text('HLH Assessoria e Consultoria em Saúde', pageWidth / 2, y, { align: 'center' });
    y += 15;

    // Filtros aplicados
    doc.setFontSize(10);
    doc.text(`Período: ${formatDate(filters.startDate)} a ${formatDate(filters.endDate)}`, 15, y);
    y += 6;
    doc.text(`Módulo: ${filters.module || 'Todos'} | Status: ${filters.status || 'Todos'}`, 15, y);
    y += 10;

    // Tabela (exemplo simples - você expande conforme o módulo)
    doc.autoTable({
        startY: y,
        head: [['Município', 'Status', 'Última Atualização', 'Observação']],
        body: data.map(item => [
            item.municipality || item.municipio || '-',
            item.status || '-',
            formatDate(item.date || item.lastUpdate),
            (item.observation || item.obs || '').substring(0, 80) + '...'
        ]),
        theme: 'grid',
...
i <= pageCount; i++) {
        doc.setPage(i);
        doc.setFontSize(8);
        doc.text(`Gerado em ${new Date().toLocaleString('pt-BR')} - SIGP Saúde v4.4`, 15, doc.internal.pageSize.getHeight() - 10);
...
function downloadPDF() {
    if (!currentReportPDF) return;
    const url = URL.createObjectURL(currentReportPDF);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Relatorio_SIGP_${new Date().toISOString().slice(0,10)}.pdf`;
    a.click();
}
...
function clearReportFilters() {
    document.querySelectorAll('#custom-reports-page input, #custom-reports-page select').forEach(el => {
        if (el.type === 'checkbox') el.checked = false;
        else el.value = '';
    });
}

function populateReportMunicipios() {
    const select = document.getElementById('report-municipios');
    if (!select) return;

    // Limpa opções antigas
    select.innerHTML = '<option value="">Todos os municípios</option>';

    // Pega a lista real do seu sistema (você já tem essa variável)
    const lista = municipalities || appState?.municipalities || [];

    lista.forEach(m => {
        const opt = document.createElement('option');
        opt.value = m.name || m.municipality || m.municipio;
        opt.textContent = m.name || m.municipality || m.municipio;
