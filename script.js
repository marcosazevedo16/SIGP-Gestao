// ============================================================================
// SIGP SAÚDE v18.0 - VERSÃO FINAL DETALHADA (SEM COMPACTAÇÃO)
// Contém: Ajustes PDF (1-21), Exportação CSV/PDF, Gráficos e Filtros.
// ============================================================================
...
// Variáveis para controle de instâncias de gráficos (evita sobreposição)
...
// ----------------------------------------------------------------------------
// 6. INJEÇÃO DE CAMPOS DINÂMICOS (REGRAS PDF)
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// 11. SOLICITAÇÕES (PDF Itens 2, 4, 5, 17)
// ----------------------------------------------------------------------------
...
function savePresentation(e) {
    ...
    if (status === 'Realizada') {
        if (formasSel.length === 0) { alert('Selecione pelo menos uma Forma.'); return; }
...
function updatePresentationCharts(data) {
    ...
    // (Outros gráficos seguem a mesma lógica, omitidos por brevidade mas mantidos na ideia geral)
}
...
function clearPresentationFilters() {
    ['filter-presentation-municipality','filter-presentation-status','filter-presentation-requester','filter-presentation-orientador'].forEach(function(id) {
        if(document.getElementById(id)) document.getElementById(id).value = '';
    });
    renderPresentations();
}
...
function exportDemandsCSV() {
    ...
    const rows = data.map(function(d) { return [formatDate(d.date), d.priority, d.status, d.description, d.user, formatDate(d.dateRealization)]; });
...
function generateDemandsPDF() {
    ...
    const rows = data.map(function(d) { return [formatDate(d.date), d.priority, d.status, d.description]; });
...
function getFilteredProductions() {
    ...
    const statusMap = { 'Pendente': 1, 'Enviada': 2, 'Cancelada': 3 };
    return filtered.sort(function(a,b) { return statusMap[a.status] - statusMap[b.status]; });
}
...
// ----------------------------------------------------------------------------
// 16. VERSÕES, GESTÃO DE USUÁRIOS E CONFIGS
// ----------------------------------------------------------------------------
...
function showMunicipalityListModal(id=null) {
    ...
    if (id) {
        const m = municipalitiesList.find(function(x) { return x.id === id; });
...
function saveMunicipalityList(e) {
    ...
    if (editingId) {
        const i = municipalitiesList.findIndex(function(x) { return x.id === editingId; });
...
function renderMunicipalityList() {
    ...
    const rows = municipalitiesList.map(function(m) {
        return '<tr><td>' + m.name + '</td><td>' + m.uf + '</td><td><button class="btn btn--sm" onclick="showMunicipalityListModal(' + m.id + ')">✏️</button> <button class="btn btn--sm" onclick="deleteMunicipalityList(' + m.id + ')">🗑️</button></td></tr>';
    }).join('');
    c.innerHTML = '<table><thead><th>Nome</th><th>UF</th><th>Ações</th></thead><tbody>' + rows + '</tbody></table>';
}
function deleteMunicipalityList(id) { if (confirm('Excluir?')) { municipalitiesList = municipalitiesList.filter(function(x) { return x.id !== id; }); salvarNoArmazenamento('municipalitiesList', municipalitiesList); renderMunicipalityList(); updateGlobalDropdowns(); } }
...
function saveUser(e) {
    ...
    if (!editingId && users.some(function(u) { return u.login === login; })) { alert('Já existe!'); return; }
    const data = {
        login: login,
    ...
    if (!editingId) {
        ...
        const i = users.findIndex(function(x) { return x.id === editingId; });
...
function renderUsers() {
    ...
    const rows = users.map(function(u) {
        return '<tr><td>' + u.login + '</td><td>' + u.name + '</td><td>' + u.status + '</td><td><button class="btn btn--sm" onclick="showUserModal(' + u.id + ')">✏️</button> <button class="btn btn--sm" onclick="deleteUser(' + u.id + ')">🗑️</button></td></tr>';
    }).join('');
...
// Cargos, Orientadores, Módulos, Formas (Implementação Padrão)
...
function handleBackupFileSelect(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (backup.data && confirm('ATENÇÃO: Isso substituirá TODOS os dados. Continuar?')) {
